...
function Home() {
 // useState est un hook de React qui permet à un composant de 
  ...
  const [boxes, setBoxes] = useState(0); // État pour stocker les divs
  ...
  const addBox = () => {
    setBoxes(boxes + 1); // Ajoute la couleur actuelle au tableau
  };
    ...
    <div className="App">
      ...
      <div className='formBonjour'>
        ...
        <div className="carreList">
            {Array.from({ length : boxes }).map((boxColor, index) => (
